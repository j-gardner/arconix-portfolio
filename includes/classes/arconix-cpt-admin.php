...
abstract class Arconix_CPT_Admin {
    ...
    public function updated_messages( $messages ) {
        ...
        $messages[ $this->post_type_name ] = array(
            0  => '',
            1  => sprintf( __( '%s updated.', 'arconix-portfolio' ), $singular ),
            2  => __( 'Custom field updated.', 'arconix-portfolio' ),
            3  => __( 'Custom field deleted.', 'arconix-portfolio' ),
            4  => sprintf( __( '%s updated.', 'arconix-portfolio' ), $singular ),
            5  => isset( $_GET[ 'revision' ] ) ? sprintf( __( '%2$s restored to revision from %1$s', 'arconix-portfolio' ), wp_post_revision_title( (int) $_GET[ 'revision' ], false ), $singular ) : false,
            6  => sprintf( __( '%s updated.', 'arconix-portfolio' ), $singular ),
            7  => sprintf( __( '%s saved.', 'arconix-portfolio' ), $singular ),
            8  => sprintf( __( '%s submitted.', 'arconix-portfolio' ), $singular ),
            9  => sprintf( __( '%2$s scheduled for: <strong>%1$s</strong>.', 'arconix-portfolio' ), date_i18n( __( 'M j, Y @ G:i', 'arconix-portfolio' ), strtotime( $post->post_date ) ), $singular ),
            10 => sprintf( __( '%s draft updated.', 'arconix-portfolio' ), $singular ),
        );
        return $messages;
    }
